...
5. [Analytics Interactions](#analytics-interactions)
...
class AnimatedCharts {
  ...
  renderQualityDonut(data) {
    ...
    segments.forEach((segment, index) => {
      ...
      segment.style.strokeDashoffset = `${25 - offset}`;
      segment.style.animation = `drawSegment 1s ease ${index * 0.1}s forwards`;
    });
    
    // Counter animation
    const counters = document.querySelectorAll('.segment-counter');
    counters.forEach((counter, index) => {
      this.animateCounter(counter, 0, data[index].count, 1000);
    });
  }
...
class PeriodSelector {
  constructor(element) {
    this.element = element;
    this.buttons = element.querySelectorAll('.period-btn');
    this.indicator = element.querySelector('.period-indicator');
  ...
  selectPeriod(button) {
    ...
    // Move indicator
    this.moveIndicator(button);
    
    // Load new data with transition
    this.transitionData(button.dataset.period);
  }
  ...
  transitionData(period) {
    const chartContainer = document.querySelector('.chart-container');
    
    // Fade out current
    chartContainer.style.animation = 'fadeOut 0.2s ease forwards';
    ...
    setTimeout(() => {
      ...
      this.loadPeriodData(period).then(data => {
        ...
        // Fade in new
        chartContainer.style.animation = 'fadeIn 0.3s ease forwards';
      });
    }, 200);
  }
}
