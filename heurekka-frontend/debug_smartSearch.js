...
// Property type mappings (Spanish)
const PROPERTY_TYPE_KEYWORDS = {
  apartamento: ['apartamento', 'apartamentos', 'apto', 'aptos', 'depto', 'deptos'],
  casa: ['casa', 'casas', 'vivienda', 'viviendas'],
  studio: ['studio', 'studios', 'estudio', 'estudios'],
  townhouse: ['townhouse', 'townhouses', 'casa adosada', 'casas adosadas'],
  condo: ['condo', 'condos', 'condominio', 'condominios']
};
...
function extractLocation(query: string): string | undefined {
  console.log("DEBUG extractLocation input:", query);
  const lowerQuery = query.toLowerCase();
    ...
    if (index !== -1) {
      ...
      if (locationMatch && locationMatch[1]) {
        ...
        if (!isPropertyKeyword && !extractNumbers(potentialLocation).length) {
          console.log("DEBUG: Found location from keyword:", potentialLocation);
    ...
    if (lastWord.match(/^[A-ZÁÉÍÓÚÑÜ]/)) {
      ...
      if (!isCommonKeyword && !extractNumbers(lastWord).length) {
        ...
        if (secondLastWord.match(/^[A-ZÁÉÍÓÚÑÜ]/) && !extractNumbers(secondLastWord).length) {
          console.log("DEBUG: Found location from end (double):", `${secondLastWord} ${lastWord}`);
          return `${secondLastWord} ${lastWord}`;
        }
        console.log("DEBUG: Found location from end (single):", lastWord);
        return lastWord;
      }
    }
  }
...
function debugSmartSearch(query: string): void {
